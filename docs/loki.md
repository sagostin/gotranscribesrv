...
For WebSocket sessions (`/ws/asr`, `/v1/listen`, `/v1/recognize`), a per-session UUID is generated at handler entry and used as the `request_id` for every event in that session.
