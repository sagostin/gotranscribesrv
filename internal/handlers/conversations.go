...
func (h *ConversationsHandler) Create(c *fiber.Ctx) error {
	...
	if err := h.db.Create(&conv).Error; err != nil {
		return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to create conversation")
	}
	if err := appendConversationItems(h.db, conv.ID, req.Items); err != nil {
		return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to store conversation items")
	}
...
func (h *ConversationsHandler) Update(c *fiber.Ctx) error {
	...
	if len(req.Metadata) > 0 && string(req.Metadata) != "null" {
		...
		if err := h.db.Model(conv).Update("metadata", conv.Metadata).Error; err != nil {
			return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to update conversation")
		}
	}
	return c.JSON(conversationJSON(conv))
}
...
func (h *ConversationsHandler) Delete(c *fiber.Ctx) error {
	...
	if err := h.db.Where("conversation_id = ?", conv.ID).Delete(&models.ConversationItem{}).Error; err != nil {
		return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to delete conversation items")
	}
	if err := h.db.Delete(conv).Error; err != nil {
		return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to delete conversation")
	}
...
func (h *ConversationsHandler) ListItems(c *fiber.Ctx) error {
	...
	if err := query.Order("seq " + order).Limit(limit + 1).Find(&rows).Error; err != nil {
		return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to list items")
	}
...
func (h *ConversationsHandler) CreateItems(c *fiber.Ctx) error {
	...
	if err := appendConversationItems(h.db, conv.ID, req.Items); err != nil {
		return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to store items")
	}
	return c.JSON(fiber.Map{
		"object":   "list",
		"data":     req.Items,
...
func (h *ConversationsHandler) DeleteItem(c *fiber.Ctx) error {
	...
	if result.Error != nil {
		return llmError(c, fiber.StatusInternalServerError, "server_error", "failed to delete item")
	}
